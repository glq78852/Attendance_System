...
from Utils import Admin_Info, Create_Window, Connect_DataBase, Send_Mail
import datetime
...
class SignOutPage:
    def __init__(self):
        self.sign_out_window = Create_Window.create_secondary_window('发布签到', 300, 300)
        ...
        tk.Label(self.sign_out_window, text='课程：', font=('宋体', 10)).place(x=70, y=30)
        class_combobox = ttk.Combobox(self.sign_out_window, width=13)
        class_combobox['value'] = class_name[0]
        class_combobox.current(0)
        class_combobox.place(x=110, y=28)

        tk.Label(self.sign_out_window, text='年级：', font=('宋体', 10)).place(x=70, y=70)
        grade_combobox = ttk.Combobox(self.sign_out_window, width=13)
        grade_combobox['value'] = ('大一', '大二', '大三', '大四')
        grade_combobox.current(0)
        grade_combobox.place(x=110, y=68)

        tk.Label(self.sign_out_window, text='专业：', font=('宋体', 10)).place(x=70, y=110)
        major_combobox = ttk.Combobox(self.sign_out_window, width=13)
        major_combobox['value'] = major_tuple
        major_combobox.current(0)
        major_combobox.place(x=110, y=108)

        tk.Label(self.sign_out_window, text='口令：', font=('宋体', 10)).place(x=70, y=150)
        random_code_combobox = tk.Entry(self.sign_out_window, width=15)
        random_code_combobox.place(x=110, y=148)

        tk.Label(self.sign_out_window, text='时间：', font=('宋体', 10)).place(x=70, y=190)
        sign_time_combobox = tk.Entry(self.sign_out_window, width=5)
        sign_time_combobox.place(x=110, y=188)

        tk.Label(self.sign_out_window, text='秒', font=('宋体', 10)).place(x=150, y=188)

        def time_counting():
            Admin_Info.set_logout_status('no')
            Admin_Info.ip_address_reset()  # 重置ip地址的记录
            Send_Mail.send_email(self.lesson_name, '甘流奇', self.time, 'glq78852@qq.com', 'sign_out')
        ...
        def confirm_release_punch():
            ...
            try:
                ...
                if result[0][0] == 0:  # 一名老师当天不止签到一次，如果是第一堂课则创建新列，不是则不创建，直接开始签到计时
                    ...
                    cursor.execute(sql)
                self.sign_out_window.destroy()
                Admin_Info.set_logout_status('yes')  # 成功发布签到后修改为yes，代表当前正在签到时间内
        ...
        bt_confirm = tk.Button(self.sign_out_window, text='确定', command=confirm_release_punch, width=10)
        bt_confirm.place(x=115, y=230)

        self.sign_out_window.mainloop()
